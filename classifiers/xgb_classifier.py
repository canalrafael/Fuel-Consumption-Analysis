...
for fs_model in selected_features:
    ...
    if len(data_paths) > 1:
        for i in range(len(data_list)):
            ...
            plt.plot(df_test["fuel_consumption_variable"].values, label="fuel_consumption_variable")
            plt.plot(5.5 * np.ones(len(y_pred)), label="threshold")
            plt.fill_between(np.arange(len(y_pred)), 0, y_pred.max(), where=y_pred, facecolor='orange', alpha=0.5)
            plt.legend()
            plt.ylabel("Fuel consumption (l/h)")
            plt.xlabel("Time (s)")
            plt.xticks([])
            plt.title("Cluster (orange/white) by vehicle consumption")
            plt.show()
